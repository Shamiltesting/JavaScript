...
Еще примеры:
getJuice(0); // 0 - нема гостей - нема і порцій
getJuice(2); // 1 + 2 = 3
...
console.log(getJuice(0)); // 0 - нема гостей - нема і порцій
console.log(getJuice(2)); // 1 + 2 = 3
console.log(getJuice(6)); // 1 + 2 + 3 + 4 + 5 + 6 = 21


/*Get Juice With Step: А теперь напишем цикл с шагом 😎
Как ты знаешь, для конкурса с напитками понадобилось множество порций. Потому ведущий решил изменить правила.
...
При этом по-прежнему пьют все присутствующие. 
Например:
...
function getDrinksWithStep(numberOfGuests, step) {
  ...
  console.log(getDrinksWithStep(0, 5)); // 0 - нема гостей — нема і порцій
