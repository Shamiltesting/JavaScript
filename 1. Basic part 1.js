//Переменная- это именованная область памяти, в имя которой можно использовать для осуществления доступа к данным, которые находятся в переменной//
...
1. имена должны описывать и отображать предназначение данных, которые храняться в переменной
2. использовать понятные слова
3. избегайте сокращений и обревиатуры, название переменной должна быть ясна для всех.*/

/*Имеются окраничения при названии переменных. Нельзя давать имя переменной:
...
/*Если название переменной имеет два слова, то приминяется два способа:
...
Константа(const)- это вид переменной, название которой невозможно поменять после ее объявления(но есть искючения)*/
...
var whichType = false;
console.log(whichType);// мы видим, что выведет логическое значение "false", несмотря на то, что до этого инициализовали переменную под другими типами значений.
...
/*var string = "55";
var number = 5;
console.log(string + number);*/// то произойдет конкатинация, то есть будет слипание number в string 

//С логическими значениями тоже произойдет конкатинация://
...
/*Инкримент\декримент постфиксны:
let a = 5;
console.log(a++);
console.log(a);
let b = 3;
...
/*Инкримент\декримент претфиксны:
let a = 5;
console.log(++a);
console.log(a);
let b = 3;
...
/*Оператор неравенства(!=), сравнивает значения по неравеству значения. Если значения равны, то возвращает false, если не равны, то true:
console.log(0 != false)
console.log(11 != 11)
console.log(1 != true)*/
...
console.log("Jiu Jistsu" > "Jiu Jits")//если все буквы равны, но одно из слов короче, то оно будет считаться меньшим 
console.log("a" > "A")*/
